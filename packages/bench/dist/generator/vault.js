 ...
 */
export async function generateVault(config) {
...
function generateNote(rng, entities, config, folders, usedFilenames, index) {
    // Generate title
    ...
    const targetLinks = Math.max(0, Math.round(config.avgLinksPerNote + (rng.next() - 0.5) * config.avgLinksPerNote * 2));
    // Calculate target length
    const targetLength = Math.max(100, Math.round(config.avgNoteLength + (rng.next() - 0.5) * config.avgNoteLength));
    // Generate content
    const { content, wikilinks } = generateNoteContent(rng, title, entities, targetLinks, targetLength);
...
async function initializeGit(vaultPath) {
 ...
 */
export async function loadVaultConfig(preset, outputDir, seed) {
 ...
 */
export async function generateQuickVault(outputDir, noteCount, seed) {
    const config = {
        outputDir,
        noteCount,
        avgLinksPerNote: 2,
