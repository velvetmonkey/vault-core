 ...
 */
export async function writeJsonReport(result, outputDir) {
 ...
 */
export async function writeMarkdownReport(result, outputDir) {
    await fs.mkdir(outputDir, { recursive: true });
    const timestamp = result.timestamp.replace(/[:.]/g, '-');
 ...
 */
export function createComparisonReport(baseline, current) {
    const lines = [];
