 ...
 */
export async function checkIntegrity(vaultPath) {
 ...
 */
async function getAllMarkdownFiles(vaultPath) {
    const files = [];
 ...
 */
function isValidYamlish(content) {
 ...
 */
function extractFrontmatterEntities(content) {
    if (!content.startsWith('---')) {
        return [];
    }
    const endIndex = content.indexOf('---', 3);
 ...
 */
export async function generateIntegrityReport(vaultPath) {
