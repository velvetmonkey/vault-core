 ...
 */
export async function appendToSection(vaultPath, notePath, section, content) {
 ...
 */
export async function contentExistsInSection(vaultPath, notePath, section, content) {
    const fullPath = path.join(vaultPath, notePath);
    try {
 ...
 */
export async function idempotentAppendToSection(vaultPath, notePath, section, content) {
 ...
 */
export async function testNonIdempotentAppend(config) {
    ...
    try {
        ...
        const finalContent = await fs.readFile(path.join(config.vaultPath, notePath), 'utf-8');
 ...
 */
export async function testIdempotentAppend(config) {
    const startTime = Date.now();
    try {
        // Setup
 ...
 */
export async function testTimestampIdempotency(config) {
    const startTime = Date.now();
    try {
        // Setup
 ...
 */
export async function runIdempotencyTests(config) {
    const results = [];
...
//# sourceMappingURL=idempotencyTest.js.map